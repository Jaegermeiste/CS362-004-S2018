/*********************************************************************
** Program name: Random Card Test Adventurer
** Author: Jason George
** Date: 20180511
** Description: randomtestadventurer.c - implementation of test for adventurer
*********************************************************************/
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
#ifndef asserttrueEX
#define asserttrueEX(runCount, errorNum, testVal, card, c1, c2, c3, hp, b, msg) if ((testVal) != 0) { errorNum += 0; } \
else { printf(" !!! TEST FAILED on run %d at line %d with inputs %s %d, %s %d, %s %d, %s %d, %s %d, %s %d; %s\n", \
runCount, __LINE__, string(card), card, string(c1), c1, string(c2), c2, string(c3), c3, string(hp), hp, string(b), b, msg); errorNum += 1; };
#endif
...
int main(int argc, char *argv[])
{
	...
	// Method under test is cardEffect, with the card "adventurer"
	...
	while (runTestLoop > 0)
	{
		...
		asserttrueEX(runCount, errorCount, retVal > -1, adventurer, choice1, choice2, choice3, handPos, originalBonus, "error");

		// Validate function adjusted the hand as required by the rules of the card: -adventurer, + 2 others
		asserttrueEX(runCount, errorCount, testState.handCount[player] == (handCount - 1 + 2), adventurer, choice1, choice2, choice3, handPos, originalBonus, "hand");

		// Validate actions did not increase, because adventurer is terminal
		asserttrueEX(runCount, errorCount, testState.numActions <= numActions, adventurer, choice1, choice2, choice3, handPos, originalBonus, "actions");

		// Validate bonus did not change, because that is not an effect of adventurer
		asserttrueEX(runCount, errorCount, bonus == originalBonus, adventurer, choice1, choice2, choice3, handPos, originalBonus, "bonus");
		...
		asserttrueEX(runCount, errorCount, newTreasureCount == (treasureCount + 2), adventurer, choice1, choice2, choice3, handPos, originalBonus, "treasureCount");				// adventurer adds two treasures
