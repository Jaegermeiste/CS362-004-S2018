/*********************************************************************
** Program name: Random Card Test 1
** Author: Jason George
** Date: 20180511
** Description: randomtestcard1.c - implementation of test for smithy
*********************************************************************/
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
/*********************************************************************
** Description: struct gameState randomState()
**      Returns a random gameState.
*********************************************************************/
struct gameState randomState()
{
	struct gameState state = {0};
	memset(&state, 0, sizeof(struct gameState));
	unsigned i = 0, j = 0;

	state.numPlayers = ((rand() % (MAX_PLAYERS - 2 + 1)) + 2);
...
int main(int argc, char *argv[])
{
	...
	printf(" RANDOM CARD TEST: smithy\n");

    // Method under test is cardEffect, with the card "smithy"
    ...
    while (runTestLoop > 0)
    {
	    ...
	    asserttrueEX(runCount, errorCount, retVal > -1, smithy, choice1, choice2, choice3, handPos, originalBonus);

        // Validate function adjusted the hand as required by the rules of the card: -smithy, + 3 others
	    asserttrueEX(runCount, errorCount, testState.handCount[player] == (handCount - 1 + 3), smithy, choice1, choice2, choice3, handPos, originalBonus);

        // Validate actions did not increase, because smithy is terminal
	    asserttrueEX(runCount, errorCount, testState.numActions <= numActions, smithy, choice1, choice2, choice3, handPos, originalBonus);					

        // Validate bonus did not change, because that is not an effect of smithy
        asserttrueEX(runCount, errorCount, bonus == originalBonus, smithy, choice1, choice2, choice3, handPos, originalBonus);
